...
void FdoSelectionManager::init()
{
    ...
    xcb_connection_t *c = QNativeInterface::QX11Application::connection();
...
bool FdoSelectionManager::nativeEventFilter(const QByteArray &eventType, void *message, qintptr *)
{
    ...
    if (responseType == XCB_CLIENT_MESSAGE) {
        ...
        if (sniProxy) {
            sniProxy->update();
            xcb_damage_subtract(QNativeInterface::QX11Application::connection(), m_damageWatches[damagedWId], XCB_NONE, XCB_NONE);
        }

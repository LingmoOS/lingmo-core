...
Daemon::Daemon(const QList<std::tuple<QString, QStringList,QProcessEnvironment>> &processList,
...
void Daemon::onProcessError(QProcess::ProcessError error) {
  ...
  for (const auto &processInfo : m_processList) {
    if (std::get<0>(processInfo) == program) {
...
void Daemon::startProcess(const std::tuple<QString, QStringList,QProcessEnvironment> &processInfo) {
  ...
  process->setProcessEnvironment(std::get<2>(processInfo));

  process->start(std::get<0>(processInfo), std::get<1>(processInfo));
  if (process->waitForStarted()) {
    qDebug() << "Process started:" << std::get<0>(processInfo)
             << "PID:" << process->processId();
  } else {
    qDebug() << "Failed to start process:" << std::get<0>(processInfo)
             << process->errorString();
  }
}
